...
use App\Actions\Debts\RegisterBorrowingAction;
...
final class RegisterBorrowingFilamentAction
{
    use HasTransactionFormComponents;

    public static function make(): Action
    {
        return Action::make('registerBorrowing')
            ->label('Pedir Prestado')
            ->icon('heroicon-o-arrow-up-circle')
            ->color('danger')
            ->modalHeading('Pedir Prestado')
            ->schema([
                Select::make('target_account_id')
                    ->label('Cuenta de Préstamo por Pagar')
                    ->options(static fn (): array => self::getAccountOptionsBySubtype(LedgerAccountSubType::LOAN_PAYABLE))
                    ->searchable()
                    ->preload()
                    ->required()
                    ->native(false)
            ...
            ->action(function (array $data): void {
                ...
                $action = resolve(RegisterBorrowingAction::class);
                $action->execute($user, $registerDebtData);

                Notification::make()
                    ->title('Deuda registrada')
                    ->body('La deuda se ha registrado correctamente')
                    ->success()
                    ->send();
            });
    }
