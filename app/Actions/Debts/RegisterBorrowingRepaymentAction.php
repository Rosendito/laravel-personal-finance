...
final readonly class RegisterBorrowingRepaymentAction
{
    ...
    {
        $loanPayableAccount = $this->findAccountForUser($user, $data->target_account_id);
        $liquidAccount = $this->findAccountForUser($user, $data->contra_account_id);

        $this->validateLoanPayableAccount($loanPayableAccount);
        $this->validateLiquidAccount($liquidAccount);
        $this->validateCurrencyMatch($loanPayableAccount, $liquidAccount);
        ...
        $transactionData = LedgerTransactionData::from([
            ...
            'entries' => [
                ...
                [
                    'account_id' => $loanPayableAccount->id,
                    'amount' => $amount,
                ],
            ],
        ]);
